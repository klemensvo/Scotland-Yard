...
public class ServerListener extends Listener {
    ...
    final private ServerData serverData;
    ...
    public void connected(Connection connection) {
        Log.info(TAG, "Player connected: " + connection.toString() +
                " from endpoint " + connection.getRemoteAddressTCP().toString());

        if (serverData.connectPlayer()) {
            Log.info(TAG, "Player " + connection + "joined the server.");
            connection.sendTCP(new PlayerConnected());
        } else {
            Log.error(TAG, "Connection: " + connection + " Lobby already full! ");
